var app = (function (L$1) {
	...
	function fillPattern(ref) {
		var item = ref.item;

		var st = L.extend({}, item.RenderStyle, {fillOpacity: 1}),
	...
	var methods = {
		clickOnExpander: function clickOnExpander(target) {
			...
			if (line.type === 'group' && !line.content.properties.ShowCheckbox) {
				...
				if (node) {
					var isCollapse = node.classList.contains('collapse'),
	...
	EventTarget.prototype.off = function off (type, callback) {
	...
	var Sidebar = /*@__PURE__*/(function (EventTarget$$1) {
	    ...
	    if ( EventTarget$$1 ) Sidebar.__proto__ = EventTarget$$1;
	    Sidebar.prototype = Object.create( EventTarget$$1 && EventTarget$$1.prototype );
	    Sidebar.prototype.constructor = Sidebar;

	    var prototypeAccessors = { tabs: { configurable: true },panels: { configurable: true },visible: { configurable: true },selected: { configurable: true } };
	    Sidebar.prototype._onTabClick = function _onTabClick (id, e) {
	        e.stopPropagation();
	        if (this.selected === id) {
	    ...
	    Sidebar.prototype.disable = function disable (id) {
	        if (this.tabs[id]) {
	            this.tabs[id].setAttribute('disabled', 'disabled');            
	        }        
	    };
	    Sidebar.prototype.enable = function enable (id) {
	        if (this.tabs[id]) {
	            this.tabs[id].removeAttribute('disabled');
	        }
	    };
	    Sidebar.prototype.enabled = function enabled (id) {        
	        return this.tabs[id] && !this.tabs[id].hasAttribute('disabled');
	    };
	    prototypeAccessors.tabs.get = function () {
	        return this._tabs;
	    };
	    prototypeAccessors.panels.get = function () {
	        return this._panels;
	    };
	    Sidebar.prototype.addTab = function addTab (id) {
	    ...
	    Sidebar.prototype.removeTab = function removeTab (id) {
	    ...
	    prototypeAccessors.visible.set = function (visible) {
	        ...
	        Object.keys(this._tabs).forEach(function (id) {
	            if (visible && id === this$1.selected) {
	                this$1._panels[id].classList.remove('hidden');                
	            }
	            else {
	                this$1._panels[id].classList.add('hidden');
	            }
	        });            
	    ...
	    prototypeAccessors.selected.set = function (selected) {
	        ...
	        if (this.selected === selected) {            
	            ...
	            Object.keys(this._tabs).forEach(function (id) {
	                if (id === selected) {
	                    this$1._tabs[id].classList.add('selected');
	                    this$1._selected = selected;
	                }
	                else {
	                    this$1._tabs[id].classList.remove('selected');                    
	                }
	            });            
	            this.visible = true;
	            var event$1 = document.createEvent('Event');
	            event$1.initEvent('change:selected', false, false);
	            this.dispatchEvent(event$1);
	        }
	    };
	    Sidebar.prototype._render = function _render (container) {
	    ...
	    Object.defineProperties( Sidebar.prototype, prototypeAccessors );

	    return Sidebar;
	}(EventTarget));

	var SidebarControl = L$1.Control.extend({
	    ...
	    onAdd: function(map) {
	        ...
	        var fakeStop = L$1.DomEvent._fakeStop || stop;
	        L$1.DomEvent
	        ...
	        this._sidebar.addEventListener('change:selected', function (e) {
	            this$1.fire ('change', e);
	        });
		...
		createMap: function createMap(it) {
			...
			var permalinkIcon = L.control.gmxIcon({
				...
				.on('click', function (ev) {
					...
					var href = (location.origin) + "?z=" + (map.getZoom()) + "&x=" + (center.lng) + "&y=" + (center.lat) + "&dt=" + (picker.getDate().toLocaleDateString());
			...
			L.gmx.loadMap(mapID, opt).then(function(gmxMap) {
				...
				if (app.iconSidebar && app.iconSidebar !== 'false') {
					...
					if (app.treeView && app.treeView !== 'false') {
						var treePane = iconSidebar.addTab('treeView');
						...
						this.treePane = iconSidebar.getTab('treeView');
						this.treePane.innerHTML = '<div data-tab-id="treeView"><i class="icon-layers closed"></i></div>';
					}
					iconSidebar.setVisible(false);
				} else {
	...
	*/
			...
			this._treeView.on('command', function (ev) {
				...
				if (gmxLayer && !(gmxLayer instanceof L.gmx.DummyLayer)) {
					if (cmd === 'fitBounds') {
						...
						var props = gmxLayer.getGmxProperties(),
							meta = props.MetaProperties,
							getContent = function () {
								var str = '<div class="title">' + props.title + '</div>';
								if (meta.desc_long) {
						...
						if (desc) {
							...
							var gmxPopup = L.control.gmxPopup({content: desc}).openOn(map),
	...
	function onstate$2(ref) {
		...
		if (changed.urlParams) {
			var pars = L.gmxUtil.searchScriptPars('winnie_2.0.js'),
