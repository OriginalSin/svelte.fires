var app = (function (leaflet) {
	...
	function isSkiped({line, layersTree}) {
		return layersTree.skiped[line.content.properties.name];
	}

	function nameToEng({ properties }) { let title = properties.title; return window.language === 'eng' ? title.replace('СКАНЭКС (Россия)', 'SCANEX (Russia)').replace('Пожары', 'Fires') : title; }
	...
	function create_main_fragment$1(component, ctx) {
		...
		var if_block = (!ctx.isSkiped) && create_if_block$1(component, ctx);
	...
	function create_if_block$1(component, ctx) {
		...
		var if_block0 = (ctx.properties) && create_if_block_2(component, ctx);

		var if_block1 = (ctx.line.content.children) && create_if_block_1$1(component, ctx);
	...
	function create_if_block_2(component, ctx) {
		...
		var if_block0 = (ctx.line.type !== 'map') && create_if_block_9(component, ctx);

		var if_block1 = (ctx.line.type !== 'map') && create_if_block_3(component, ctx);
	...
	// (6:1) {#if line.type !== 'map'}
	function create_if_block_9(component, ctx) {
		var text, if_block1_anchor;

		var if_block0 = (ctx.line.type === 'group') && create_if_block_14(component, ctx);

		var if_block1 = (ctx.line.type === 'layer') && create_if_block_10(component, ctx);
	...
	// (11:2) {#if line.type === 'layer'}
	function create_if_block_10(component, ctx) {
		var text0, span1, text1, text2, span0, span0_title_value;

		var if_block0 = (ctx.properties.styles.length > 1) && create_if_block_13(component, ctx);

		var if_block1 = (ctx.gmxTimeline && ctx.properties.Temporal && (ctx.properties.IsRasterCatalog || (ctx.properties.Quicklook && ctx.properties.Quicklook !== 'null'))) && create_if_block_12(component, ctx);

		var if_block2 = (ctx.isInfo) && create_if_block_11(component, ctx);
		...
		return {
			c: function create() {
				...
				span0.title = span0_title_value = ctx.isEng ? 'Position' : 'Позиционировать';
				addLoc(span0, file$1, 23, 3, 1095);
				span1.className = "icons  svelte-t2dx67";
				addLoc(span1, file$1, 14, 1, 630);
			},
			...
			p: function update(changed, ctx) {
				...
				if ((changed.isEng) && span0_title_value !== (span0_title_value = ctx.isEng ? 'Position' : 'Позиционировать')) {
	...
	// (12:3) {#if properties.styles.length > 1}
	...
	// (17:2) {#if gmxTimeline && properties.Temporal && (properties.IsRasterCatalog || (properties.Quicklook && properties.Quicklook !== 'null'))}
	function create_if_block_12(component, ctx) {
		var span, span_title_value;
		...
		return {
			c: function create() {
				...
				span.title = span_title_value = ctx.isEng ? 'Add to timeline' : 'Добавить на таймлайн';
				addLoc(span, file$1, 17, 2, 791);
			},
			...
			p: function update(changed, ctx) {
				if ((changed.isEng) && span_title_value !== (span_title_value = ctx.isEng ? 'Add to timeline' : 'Добавить на таймлайн')) {
	...
	function create_if_block_11(component, ctx) {
		...
		return {
			c: function create() {
				...
				span.title = span_title_value = ctx.isEng ? 'Layer info' : 'Метаданные слоя';
				addLoc(span, file$1, 21, 2, 982);
			},
			...
			p: function update(changed, ctx) {
				if ((changed.isEng) && span_title_value !== (span_title_value = ctx.isEng ? 'Layer info' : 'Метаданные слоя')) {
					span.title = span_title_value;
				}
			},
	...
	// (30:1) {#if line.type !== 'map'}
	function create_if_block_3(component, ctx) {
		var span, text0, text1, label, text2, label_class_value, text3, div, raw_value = ctx.properties.description || '', div_class_value, text4, current;

		function select_block_type(ctx) {
			if (ctx.line.type === 'group') return create_if_block_7;
		...
		var if_block1 = (ctx.line.type === 'layer' && ctx.styles.length && ctx.styles.length < 2 && ctx.styles[0].RenderStyle) && create_if_block_6(component, ctx);

		function click_handler(event) {
			component.clickOnExpander(this);
		}

		var if_block2 = (ctx.line.type === 'layer') && create_if_block_4(component, ctx);
		...
		return {
			c: function create() {
				...
				text2 = createText(ctx.nameToEng);
				text3 = createText("\n\t\t");
				div = createElement("div");
				text4 = createText("\n\t\t");
				if (if_block2) if_block2.c();
			...
			p: function update(changed, ctx) {
				...
				if (!current || changed.nameToEng) {
					setData(text2, ctx.nameToEng);
				}
	...
	// (32:2) {#if line.type === 'group'}
	function create_if_block_7(component, ctx) {
		var if_block_anchor;

		var if_block = (ctx.properties.ShowCheckbox) && create_if_block_8(component, ctx);
	...
	// (33:3) {#if properties.ShowCheckbox}
	...
	// (39:2) {#if line.type === 'layer' && styles.length && styles.length < 2 && styles[0].RenderStyle}
	...
	// (49:2) {#if line.type === 'layer'}
	function create_if_block_4(component, ctx) {
		var if_block_anchor, current;

		var if_block = (ctx.styles.length && ctx.styles.length > 1 && ctx.styles[0].RenderStyle) && create_if_block_5(component, ctx);
	...
	// (50:3) {#if styles.length && styles.length > 1 && styles[0].RenderStyle}
	function create_if_block_5(component, ctx) {
		...
		return {
			c: function create() {
				...
				addLoc(div0, file$1, 51, 4, 2328);
				div1.className = "legend svelte-t2dx67";
				addLoc(div1, file$1, 50, 3, 2303);
			},
	...
	function create_each_block_1(component, ctx) {
		...
		return {
			c: function create() {
				...
				addLoc(span0, file$1, 54, 6, 2421);
				span1.className = "styleName svelte-t2dx67";
				addLoc(span1, file$1, 56, 6, 2561);
				div.className = "gmx-style-legend svelte-t2dx67";
				addLoc(div, file$1, 53, 5, 2384);
			},
	...
	// (71:1) {#each line.content.children || [] as child}
	...
	LineNode.prototype._checkReadOnly = function _checkReadOnly(newState) {
		...
		if ('isSkiped' in newState && !this._updatingReadonlyProperty) throw new Error("<LineNode>: Cannot set read-only property 'isSkiped'");
		if ('nameToEng' in newState && !this._updatingReadonlyProperty) throw new Error("<LineNode>: Cannot set read-only property 'nameToEng'");
		if ('isEng' in newState && !this._updatingReadonlyProperty) throw new Error("<LineNode>: Cannot set read-only property 'isEng'");
	};
	...
	LineNode.prototype._recompute = function _recompute(changed, state) {
		...
		if (changed.line || changed.layersTree) {
			...
			if (this._differs(state.isSkiped, (state.isSkiped = isSkiped(state)))) changed.isSkiped = true;
		}

		if (changed.properties) {
			if (this._differs(state.nameToEng, (state.nameToEng = nameToEng(state)))) changed.nameToEng = true;
		}

		if (changed.line) {
			if (this._differs(state.isEng, (state.isEng = isEng(state)))) changed.isEng = true;
		}
	};
				...
				if (app.gmxTimeline && app.gmxTimeline !== 'false') {
					...
					repaintTimeline = () => {
						if (control._container) {
							control._container.classList['remove']('shiftTimeline');
	...
	function create_if_block$3(component, ctx) {
		...
		return {
			c: function create() {
				...
				addLoc(rect, file$5, 28, 31, 816);
				setAttribute(clippath, "id", "clip-path");
				addLoc(clippath, file$5, 28, 6, 791);
				addLoc(defs, file$5, 26, 2, 775);
				addLoc(title, file$5, 31, 6, 938);
